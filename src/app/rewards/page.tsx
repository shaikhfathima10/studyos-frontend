...
import { Home, Calendar, Clock, BookOpen, TrendingUp, Trophy, Flame, Diamond, Users } from "lucide-react";
...
export default function RewardsPage() {
  ...
  const api  = (path: string) =>
    fetch(BASE + path, { headers: { "Content-Type": "application/json", Authorization: "Bearer " + tok() } }).then(r => r.json());

  useEffect(() => {
    if (!tok()) { router.push("/"); return; }
    (async () => {
      try { const [b, s] = await Promise.all([api("/api/rewards/badges"), api("/api/rewards/streak")]); setBadges(Array.isArray(b) ? b : []); setStreak(s); }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8FAFC", maxWidth: 480, margin: "0 auto", paddingBottom: 100 }}>
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: "#fff", borderBottom: "1px solid #E2E8F0", padding: "16px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)" }}>
        <h1 style={{ margin: 0, fontSize: 20, fontWeight: 900, color: "#1E293B", display: "flex", alignItems: "center", gap: 8 }}><Trophy size={20} color="#1E40AF" /> Rewards</h1>
        <p style={{ margin: 0, color: "#64748B", fontSize: 12 }}>Streaks, XP and Badges</p>
      </div>
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
          {[
            { Icon: Flame,   label: "Current Streak", val: (streak?.streak || 0) + " days",         color: "#D97706", bg: "#FEF3C7" },
            { Icon: Diamond, label: "Longest Streak", val: (streak?.longest_streak || 0) + " days",  color: "#7C3AED", bg: "#EDE9FE" },
          ].map(({ Icon, label, val, color, bg }) => (
            <div key={label} style={{ ...card, background: bg, border: "1px solid " + color + "22", marginBottom: 0 }}>
              <Icon size={28} color={color} />
              <p style={{ margin: "8px 0 0", fontSize: 22, fontWeight: 900, color }}>{val}</p>
              <p style={{ margin: "2px 0 0", fontSize: 11, color, fontWeight: 600 }}>{label}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ ...card, background: "#EFF6FF", border: "1px solid #BFDBFE" }}>
          <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 14 }}>
            ...
            <div>
              ...
              <p style={{ margin: 0, fontSize: 12, color: "#64748B" }}>Study together and earn bonus XP</p>
            </div>
          </div>
          <button style={{ width: "100%", padding: 14, borderRadius: 12, border: "none", background: "linear-gradient(135deg, #1E40AF, #4338CA)", color: "#fff", fontWeight: 800, fontSize: 15, cursor: "pointer" }}>
            Create Study Room
          </button>
        </div>
      </div>
